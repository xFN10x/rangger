...
console.stdout:writeln("\nrangger (Rangger) 1.0- Range Compressing softwave.----------------------------------------------------------------")
console.stdout:writeln("Copyright © 2023-2024 FN10\n")
...
local log = sys.File("log.conlog"):open("write")
--Important Varibles
...
local function CompressFile(File)
  ...
  print("prline:29 "..workingfolder.fullpath)
  ...
  if cmd == true then
    ...
    if sys.File("out/"..File.name).exists == false then
      ...
      if outputfile == nil then
        print("prline:51 "..workingfolder.fullpath..'/'.."out"..File.extension,outputfile)
        print("prline:52 "..sys.currentdir)
        ui.error("Failed to range compress. Retrying CMDs (failed to copy to file:ec2)")
        CompressFile(File)
      else
        ...
        if ui.confirm("Do you want to listen to the outputed file?") == "yes" then
          sys.cmd('"'..outputfile.fullpath..'"',false,true)
        end
        print("prline:71",workingfolder:removeall())
      end
    else
...
local smbypass = ui.Checkbox(tabs.items[1],"Disable Errors",105,120)
smimgfullbutton:loadicon("img/full.ico")
smfiletext:show()
--Single mode functions
function smbypass.onClick()
  if not smbypass.checked == false then
    ui.warn("Enabling this could break the app.")
  end
end
...
console.stdout:copy(sys.currentdir.."/log.conlog")
